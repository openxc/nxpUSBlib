			                                ...
			                                uint16_t* const BytesProcessed)
{
	...
	if (endpointselected==ENDPOINT_CONTROLEP){
		...
		if(usb_data_buffer_OUT_sizes[endpointselected] == 0) return ENDPOINT_RWSTREAM_IncompleteTransfer;

	for(i=0;i<Length;i++)
	{
		#if defined(__LPC17XX__) || defined(__LPC177X_8X__)
		if (endpointselected!=ENDPOINT_CONTROLEP)
			while(usb_data_buffer_OUT_sizes[endpointselected] == 0); /* Current Fix for LPC17xx, havent checked for others */
		#endif
