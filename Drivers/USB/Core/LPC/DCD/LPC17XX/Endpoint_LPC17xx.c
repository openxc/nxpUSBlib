...
void HAL_Reset (void)
{
    ...
    for(n = 0; n < ENDPOINT_TOTAL_ENDPOINTS; n++) {
        usb_data_buffer_OUT_sizes[n] = 0;
        usb_data_buffer_OUT_indexes[n] = 0;
        usb_data_buffer_IN_indexes[n] = 0;
    }
	...
	for (PhyEP = 2; PhyEP < USED_PHYSICAL_ENDPOINTS; PhyEP++)              /* Check All Endpoints */
	{
		if ( EoTIntSt & _BIT(PhyEP) )
		{
			if ( IsOutEndpoint(PhyEP) )                 /* OUT Endpoint */
			{
				...
				usb_data_buffer_OUT_sizes[LOGICAL_ENDPOINT(PhyEP)] += dmaDescriptor[PhyEP].PresentCount;
				if((usb_data_buffer_OUT_sizes[LOGICAL_ENDPOINT(PhyEP)] + usb_data_buffer_OUT_indexes[LOGICAL_ENDPOINT(PhyEP)] +
